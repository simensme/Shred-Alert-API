...
const compareMonitorToAPI = async () => {
	const getWeather = await getWeatherData(
		'59.92160104865082',
		'10.744014548914478'
	);
	...
	for (let i = 0; i < weatherToObjArr.length; i++) {
		...
		console.log(
			`Temperature: ${Number(
				Object.values(weatherToObjArr[i].parameters[0])
			)}, Precipitation: ${Number(
				Object.values(weatherToObjArr[i].parameters[1])
			)}, Windspeed: ${Number(Object.values(weatherToObjArr[i].parameters[2]))}`
		);
